...
DROP VIEW IF EXISTS gn_monitoring.v_synthese_steli;
CREATE VIEW gn_monitoring.v_synthese_steli AS
WITH srce AS (
		SELECT 
			sc.id_source,
			mo.id_module
		...
		WHERE sc.name_source = 'MONITORING_STELI'
	),
	sites AS (
		SELECT
			tbs.id_base_site,
	...
	SELECT
		...
		ref_nomenclatures.get_id_nomenclature('METH_OBS', '0') AS id_nomenclature_obs_technique, -- METH_OBS
		...
		COALESCE((toc.data ->> 'effectif'):: integer,1) AS count_min,
		COALESCE((toc.data ->> 'effectif'):: integer,1) AS count_max,
		id_observation,
		o.cd_nom,
