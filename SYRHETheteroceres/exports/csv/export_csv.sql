...
DROP VIEW  IF EXISTS  gn_monitoring.v_export_syrhetheteroceres_obs;

CREATE OR REPLACE VIEW gn_monitoring.v_export_syrhetheteroceres_obs AS
...
WITH  
	...
	visits AS (
		...
		WHERE LOWER(tm.module_code) = 'syrhetheteroceres'
	), 
	observers AS (
		SELECT 
 ...
 SELECT 
    ...
    v.date_min,
    v.date_max,
	ref_nomenclatures.get_nomenclature_label((v.visit_data ->> 'meteo'::text)::integer) AS meteo,
	ref_nomenclatures.get_nomenclature_label((v.visit_data ->> 'pluviosite'::text)::integer) AS pluviosite,
	ref_nomenclatures.get_nomenclature_label((v.visit_data ->> 'vent'::text)::integer) AS vent,
	(visit_data ->> 'temperature'::text)::integer AS temperature,
	obs.observers,
    v.numerisateur,
    d.nom_role || ' ' ||d.prenom_role AS determiner,
    o.cd_nom,
    t.nom_complet ,
	(o_compl.data->>'conservation') as conservation_echantillon,
	(o_compl.data->>'id_echantillon')::integer as id_echantillon,
	(o_compl.data ->> 'effectif'::text)::integer AS count_min,
    (o_compl.data ->> 'effectif'::text)::integer AS count_max,
    ref_nomenclatures.get_nomenclature_label((o_compl.data ->> 'id_nomenclature_life_stage'::text)::integer) AS stade_vie,
    ref_nomenclatures.get_nomenclature_label((o_compl.data ->> 'id_nomenclature_sex'::text)::integer) AS sexe,
	ref_nomenclatures.get_nomenclature_label_by_cdnom_mnemonique('TECHNIQUE_OBS'::character varying, '94'::character varying) AS technique_collecte,
	ref_nomenclatures.get_nomenclature_label_by_cdnom_mnemonique('TYP_GRP'::character varying, 'REL'::character varying) AS type_regroupement,
	ref_nomenclatures.get_nomenclature_label_by_cdnom_mnemonique('METH_OBS'::character varying, '0'::character varying) AS technique_obs,
	ref_nomenclatures.get_nomenclature_label_by_cdnom_mnemonique('METH_DETERMIN'::character varying, '14'::character varying) AS methode_determination,
...
LEFT JOIN utilisateurs.t_roles d ON (o_compl.data ->> 'determiner')::integer = id_role
ORDER BY 
	site,
	piege,
	date_min,
