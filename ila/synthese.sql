...
CREATE VIEW gn_monitoring.v_synthese_:module_code AS
 ...
 SELECT o.uuid_observation AS unique_id_sinp,
    ...
    s.altitude_min,
    s.altitude_max,
    ((((((s.data ->> 'commune'::text) || ' - '::text) || s.sites_group_code::text) || ' - '::text) || s.sites_group_name) || ' - '::text) || s.base_site_code::text AS place_name,
    s.the_geom_4326,
    s.the_geom_point,
    ...
    v.id_module,
    (
		'passage : ' || (v.data ->> 'num_passage')
		|| ' | habitat : ' || (v.data ->> 'habitat')
		|| ' | gestion : '::text || COALESCE(v.data ->> 'gestion', '/')
		|| ' | impact : '::text || COALESCE((v.data ->> 'impact'), '/')
		|| ' | vent : '	|| ref_nomenclatures.get_nomenclature_label((v.data ->> 'id_nomenclature_vt')::integer)
		|| ' | couverture nuageuse : ' || ref_nomenclatures.get_nomenclature_label((v.data ->> 'id_nomenclature_cn')::integer) 
